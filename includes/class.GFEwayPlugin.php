...
final class GFEwayPlugin {
	...
	public function load() : void {
		...
		if (has_required_gravityforms()) {
			...
			add_action('gform_entry_post_save', [$this, 'gformEntryPostSave']);
			add_filter('gform_custom_merge_tags', [$this, 'gformCustomMergeTags'], 10, 3);
			add_filter('gform_replace_merge_tags', [$this, 'gformReplaceMergeTags'], 10, 5);
			add_filter('gform_entry_meta', [$this, 'gformEntryMeta']);
	...
	public function registerScripts() : void {
		...
		wp_register_script('gfeway-ecrypt', plugins_url("static/js/gfeway_ecrypt$min.js", GFEWAY_PLUGIN_FILE), ['jquery','eway-ecrypt'], null, true);
	 ...
	 * @param boolean $ajax
	 */
	public function gformEnqueueScripts($form, $ajax) : void {
	 ...
	 */
	public function ecryptInitScript() : void {
	...
	public function ecryptModifyForm($form) {
		if ($this->canEncryptCardDetails($form)) {
			...
			add_filter('gform_get_form_filter_' . $form['id'], [$this, 'ecryptEndRender']);
	 ...
	 */
	public function ecryptEndRender(string $html) : string {
		remove_filter('gform_form_tag', [$this, 'ecryptFormTag']);
	 ...
	 */
	public function ecryptFormTag(string $tag) : string {
	 ...
	 */
	private function getTestCardNumber($supportedCards) : string {
		...
		return $testNumbers[$cardType] ?? $testNumbers['visa'];
	}
	 ...
	 * @param array $form
	 */
	private function hasFormBeenProcessed($form) : bool {
	 ...
	 */
	private function getPaymentRequestor(string $type) {
		$eway = null;
	 ...
	 */
	private function processSinglePayment(array $data, GFEwayFormData $formData) : array {
		try {
	 ...
	 */
	private function processRecurringPayment(array $data, GFEwayFormData $formData) : array {
		try {
	 ...
	 * @return array
	 */
	public function gformEntryPostSave($entry) {
	 ...
	 * @return array
	 */
	public function gformCustomMergeTags($merge_tags, $form_id, $fields) {
	 ...
	 * @return string
	 */
	public function gformReplaceMergeTags($text, $form, $lead, $url_encode, $esc_html) {
	 ...
	 */
	public function gformEntryMeta(array $entry_meta) : array {
	 ...
	 */
	private function canEncryptCardDetails(array $form) : bool {
	 ...
	 */
	public static function isEwayForm(int $form_id, array $fields) : bool {
	 ...
	 */
	public static function hasFieldType($fields, string $type) : bool {
		if (is_array($fields)) {
	 ...
	 */
	private function getEwayCredentials() : array {
	 ...
	 */
	public function getErrMsg(string $errName, bool $useDefault = false) : string {
	 ...
	 */
	public function enableLogging(array $plugins) : array {
