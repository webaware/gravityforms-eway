...
(function ($) {
  ...
  var setPickerOptions = false; // set datepicker minimum date if given

  $("input[data-gfeway-minDate]").each(function () {
    var input = $(this);
    var minDate = this.getAttribute("data-gfeway-minDate"); // if minDate is an ISO date string, convert to a Date object as a reliable way to set minDate
  ...
  $("input[data-gfeway-maxDate]").each(function () {
    var input = $(this);
    var maxDate = this.getAttribute("data-gfeway-maxDate"); // if maxDate is an ISO date string, convert to a Date object as a reliable way to set maxDate
  ...
  */
  ...
  /* , defaultValues, isInit */
  ) {
    var target = $(targetId);

    if (target.hasClass("gfeway-contains-recurring")) {
      if (action === "show") {
        target.addClass("gfeway-recurring-active").removeClass("gfeway-recurring-inactive"); // reset fields to initial values
