...
(function ($) {
  var TYPE_RECURRING = "gfewayrecurring"; // add required classes to the field on the admin form

  fieldSettings.gfewayrecurring = ".conditional_logic_field_setting, .error_message_setting, .label_setting, .admin_label_setting, .rules_setting, .description_setting, .css_class_setting, .gfewayrecurring_setting"; // binding to the load field settings event to initialize custom inputs

  $(document).on("gform_load_field_settings", function (event, field
  /* , form */
  ) {
    $("#gfeway_initial_setting").prop("checked", field.gfeway_initial_setting);
    setFieldsVisibility("#gfeway_initial_fields", field.gfeway_initial_setting); // NB: for backwards compatibility, check for combined start/end setting
    ...
    setFieldsVisibility("#gfeway_recurring_start_date_fields", dateStart); // NB: for backwards compatibility, check for combined start/end setting
  ...
  */
  ...
  $("#gfeway_recurring_date_start").on("change", function () {
    SetFieldProperty(this.id, this.checked); // cleanup old combined start/end setting
  ...
  */
  ...
  $("li.gfewayrecurring_setting").on("keyup", "input.gfeway-field-label", function () {
    var newLabel = this.value; // if new label value is empty, pick up the default value instead
